...
function RepositoryDropdown({
  ...
  return (
    <div className="space-y-2">
      ...
      {selectedRepo && (
        <p className="text-xs text-muted-foreground">
          {t('projectSections.github.selectedRepo', { repo: selectedRepo })}
        </p>
      )}
    </div>
  );
...
function ConnectionStatus({ isChecking, connectionStatus }: ConnectionStatusProps) {
  ...
  return (
    <div className="rounded-lg border border-border bg-muted/30 p-3">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-foreground">{t('integrations.github.connectionStatus')}</p>
          <p className="text-xs text-muted-foreground">
            {isChecking ? t('integrations.github.checking') :
              connectionStatus?.connected
                ? t('integrations.github.connectedTo', { repo: connectionStatus.repoFullName })
                : connectionStatus?.error || t('integrations.github.notConnected')}
          </p>
    ...
    <div className="rounded-lg border border-info/30 bg-info/5 p-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="text-sm font-medium text-foreground">{t('integrations.github.issuesAvailable')}</p>
